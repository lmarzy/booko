...
export default async function handler(request: Request) {
  ...
  url.searchParams.set(
    "fields",
    "key,title,author_name,cover_i,first_sentence,number_of_pages_median,isbn",
  );
  ...
  try {
      ...
      return [
        {
          ...
          pageCount:
            Number.isInteger(item.number_of_pages_median) &&
            Number(item.number_of_pages_median) > 0
              ? (item.number_of_pages_median ?? null)
              : null,
    ...
    return Response.json(
      { error: "We couldn't reach Open Library. Please try again." },
      { status: 502 },
    );
  }
}
