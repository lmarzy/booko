...
test("routes book searches through a Netlify function", async () => {
  const client = await readFile(new URL("../lib/books.ts", import.meta.url), "utf8");
  const handler = await readFile(new URL("../netlify/functions/books-search.ts", import.meta.url), "utf8");
  assert.match(client, /\/\.netlify\/functions\/books-search/);
  assert.match(handler, /https:\/\/openlibrary\.org\/search\.json/);
  assert.match(handler, /Response\.json\(\{ books \}/);
});
