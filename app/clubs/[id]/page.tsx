...
export default function ClubPage(){
  ...
  async function inviteMembers(event:FormEvent<HTMLFormElement>){event.preventDefault();const emails=inviteEmails.split(/[\s,;]+/).map((email)=>email.trim()).filter(Boolean);if(!emails.length)return;setSendingInvites(true);setMessage("");const {data,error}=await supabase.rpc("invite_club_members",{target_club_id:clubId,invite_emails:emails});if(error)setMessage(error.message);else{setShowInvite(false);setInviteEmails("");notify(`${data} invitation${data===1?"":"s"} added. They’ll see it when they sign in.`);}setSendingInvites(false);}
  ...
  return <main className="club-page">
    ...
    {showInvite&&club.host_id===session?.user.id&&<div className="modal-backdrop invite-layer" onMouseDown={()=>!sendingInvites&&setShowInvite(false)}><section className="modal invite-modal" role="dialog" aria-modal="true" aria-labelledby="invite-title" onMouseDown={(event)=>event.stopPropagation()}><button className="close" onClick={()=>setShowInvite(false)} disabled={sendingInvites} aria-label="Close">×</button><span className="eyebrow coral">GROW THE READING CIRCLE</span><h2 id="invite-title">Invite members to {club.name}</h2><p>Enter one or more email addresses. Existing Booko users will see the invitation when they next sign in.</p><form onSubmit={inviteMembers}><label>Member emails <small>Separate multiple addresses with commas or spaces</small><textarea value={inviteEmails} onChange={(event)=>setInviteEmails(event.target.value)} placeholder="testmember@booko.test, reader@example.com" autoFocus required/></label>{message&&<p className="notice" role="status">{message}</p>}<div className="form-actions"><button type="button" className="secondary" onClick={()=>setShowInvite(false)} disabled={sendingInvites}>Cancel</button><button className="primary" disabled={sendingInvites||!inviteEmails.trim()}>{sendingInvites?"Adding…":"Add invitations →"}</button></div></form></section></div>}
  </main>;
}
