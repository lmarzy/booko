...
type ClubBook={is_current:boolean;nominated_by:string;nominated_at:string;book:SavedBook;votes:{user_id:string}[]};
...
export default function ClubPage(){
  ...
  async function loadClub(){
    ...
    if(error){setMessage("This club could not be found, or you are not a member.");return;}
    setClub(data);await loadBooks();
  }
  ...
  async function updateProgress(event:FormEvent<HTMLFormElement>){
    ...
    if(error)setMessage(error.message);else{await loadProgress(current.book.id);setValue("");notify("Your reading progress was updated.");}setSavingProgress(false);
  }
  ...
  if(checking)return <main className="loading-screen"><div className="brand"><span className="brand-mark">b</span><span>booko</span></div><p>Opening your club…</p></main>;
  ...
  return <main className="club-page">
    <header className="app-header"><a className="brand" href="/"><span className="brand-mark">b</span><span>booko</span></a><a className="back-link" href="/">← Dashboard</a><div className="account"><span className="avatar">{displayName.slice(0,2).toUpperCase()}</span><div className="account-info"><strong>{displayName}</strong><small>{session?.user.email}</small><button onClick={()=>supabase.auth.signOut().then(()=>window.location.href="/")}>Sign out</button></div></div></header>
    ...
    <div className="club-page-content">{message&&<p className="notice">{message}</p>}
      {current?<section className="current-reading-route"><div className="route-book"><BookCover title={current.book.title} url={current.book.cover_url}/><div><span className="eyebrow gold">CURRENT READ</span><h2>{current.book.title}</h2><p className="byline">{current.book.authors.join(", ")||"Unknown author"}</p>{current.book.description&&<p className="book-description">{current.book.description}</p>}<small>{current.book.page_count?`${current.book.page_count} pages`:"Page count unavailable"}</small></div></div><form className="route-progress" onSubmit={updateProgress}><div className="progress-form-heading"><div><span className="eyebrow">YOUR PROGRESS</span><small>{mine?.current_page!=null&&current.book.page_count?`Page ${mine.current_page} of ${current.book.page_count}`:"Update after each reading session."}</small></div><div className="mode-switch"><button type="button" className={mode==="page"?"active":""} disabled={!current.book.page_count} onClick={()=>setMode("page")}>Page</button><button type="button" className={mode==="percent"?"active":""} onClick={()=>setMode("percent")}>Percent</button></div></div><strong className="large-percent">{Math.round(Number(mine?.progress_percent??0))}%</strong><div className="dashboard-progress"><i style={{width:`${Number(mine?.progress_percent??0)}%`}}/></div><div className="progress-entry"><input type="number" min="0" max={mode==="page"?(current.book.page_count??undefined):100} step={mode==="page"?1:.1} required value={value} onChange={(event)=>setValue(event.target.value)} placeholder={mode==="page"?`0–${current.book.page_count??"?"}`:"0–100"}/><span>{mode==="page"?"pages":"%"}</span><button className="primary" disabled={savingProgress}>{savingProgress?"Saving…":"Update"}</button></div></form></section>:<section className="no-current-read"><span>☰</span><div><span className="eyebrow coral">CHOOSE WHAT’S NEXT</span><h2>No current book selected</h2><p>Vote on the shortlist below. The host can choose the club’s next read.</p></div></section>}
