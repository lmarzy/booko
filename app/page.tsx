...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {showLibrary&&<div className="modal-backdrop" onMouseDown={()=>setShowLibrary(false)}><section className="modal library-modal" role="dialog" aria-modal="true" aria-labelledby="library-title" onMouseDown={(event)=>event.stopPropagation()}><button className="close" onClick={()=>setShowLibrary(false)} aria-label="Close">×</button><div className="library-modal-heading"><div><span className="eyebrow coral">MY LIBRARY</span><h2 id="library-title">Books for later</h2><p>Your saved reading list, all in one place.</p></div><button className="primary" onClick={()=>openBookSearch()}>⌕ Find a book</button></div>{message&&<p className="notice">{message}</p>}{library.length===0?<button className="library-empty" onClick={()=>openBookSearch()}><span>⌕</span><strong>Search for your first book</strong><small>Find it by title, author, or ISBN.</small></button>:<div className="book-grid">{library.map((book)=><article className="book-card" key={book.id}><button className="delete-control" onClick={()=>removeLibraryBook(book)} aria-label={`Remove ${book.title}`}>×</button><BookCover title={book.title} url={book.cover_url}/><div><span className="pill">WANT TO READ</span><h3>{book.title}</h3><p className="byline">{book.authors.join(", ")||"Unknown author"}</p><small>{book.page_count?`${book.page_count} pages`:"Page count unavailable"}</small></div></article>)}</div>}</section></div>}
