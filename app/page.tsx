...
type PersonalBook = LibraryBook & { status:"want_to_read"|"reading"|"finished"; current_page:number|null; progress_percent:number; started_at:string|null; finished_at:string|null };
...
export default function Home() {
  ...
  async function loadLibrary() {
    ...
    if (error) { setMessage(error.message); return; }
    setLibrary((data ?? []).flatMap((row) => row.book ? [{...(row.book as unknown as LibraryBook),status:row.status,current_page:row.current_page,progress_percent:Number(row.progress_percent),started_at:row.started_at,finished_at:row.finished_at} as PersonalBook] : []));
  }
  ...
  async function updatePersonalProgress(event:FormEvent<HTMLFormElement>){event.preventDefault();const current=library.find((item)=>item.status==="reading");if(!current||personalValue==="")return;setSavingPersonal(true);const number=Number(personalValue);const {error}=await supabase.rpc("update_personal_reading_progress",{target_book_id:current.id,entered_page:personalMode==="page"?number:null,entered_percent:personalMode==="percent"?number:null});if(error)setMessage(error.message);else{await loadLibrary();setPersonalValue("");showTemporaryMessage(number===100&&personalMode==="percent"?"Book finished — well done!":"Your personal reading progress was updated.");}setSavingPersonal(false);}
