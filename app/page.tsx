...
type ReadingProgress = { user_id:string; display_name:string; first_name:string; last_name:string; current_page:number|null; progress_percent:number; finished_at:string|null };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, next) => { setAccountOpen(false); setSession(next); setChecking(false); });
  ...
  async function submitAuth(event:FormEvent<HTMLFormElement>) {
    ...
    if (mode === "signup") {
      const { error } = await supabase.auth.signUp({ email, password, options:{ data:{ first_name:firstName,last_name:lastName,display_name:`${firstName} ${lastName}`.trim() }, emailRedirectTo:window.location.origin } });
  ...
  return <main className="app-shell">
    <header className="app-header"><a className="brand" href="/"><span className="brand-mark">b</span><span>booko</span></a><div className="account-menu" onBlur={(event)=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setAccountOpen(false)}}><button className="avatar avatar-button" aria-label="Open account menu" aria-expanded={accountOpen} onClick={()=>setAccountOpen((open)=>!open)}>{initials(firstName,lastName)}</button>{accountOpen&&<div className="account-popover"><span className="eyebrow coral">SIGNED IN AS</span><strong>{displayName}</strong><small>{session.user.email}</small><button className="logout-button" onClick={()=>{setAccountOpen(false);void supabase.auth.signOut()}}>Log out →</button></div>}</div></header>
