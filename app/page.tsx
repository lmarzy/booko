...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {activeClub && <div className="modal-backdrop" onMouseDown={() => setActiveClub(null)}><section className="modal club-modal" role="dialog" aria-modal="true" aria-labelledby="club-title" onMouseDown={(e)=>e.stopPropagation()}><button className="close" onClick={()=>setActiveClub(null)} aria-label="Close">×</button><span className="eyebrow coral">CLUB SHORTLIST</span><div className="club-modal-heading"><div><h2 id="club-title">{activeClub.name}</h2><p>{activeClub.description || "Choose the story your club will share next."}</p></div><button className="primary" onClick={()=>openBookSearch(activeClub)}>＋ Nominate a book</button></div>{message && <p className="notice" role="status">{message}</p>}{currentClubBook&&<ProgressPanel book={currentClubBook.book} progress={readingProgress} currentUserId={session.user.id} mode={progressMode} setMode={setProgressMode} value={progressValue} setValue={setProgressValue} saving={savingProgress} submit={updateReadingProgress}/>} {clubBooks.length===0 ? <button className="shortlist-empty" onClick={()=>openBookSearch(activeClub)}><strong>No books nominated yet</strong><small>Search the catalogue and add the first contender.</small></button> : <><div className="shortlist-heading"><span className="eyebrow">{currentClubBook?"MORE NOMINATIONS":"BOOKS TO VOTE ON"}</span><small>{clubBooks.filter((item)=>!item.is_current).length} on the shortlist</small></div><div className="shortlist">{clubBooks.filter((item)=>!currentClubBook||!item.is_current).map((item)=><article className={`shortlist-book ${item.book.google_books_id===newlyNominatedId?"just-added":""}`} key={item.book.id}>{item.book.google_books_id===newlyNominatedId&&<span className="added-flag">JUST ADDED</span>}<BookCover title={item.book.title} url={item.book.cover_url}/><div><span className="pill">NOMINATED</span><h3>{item.book.title}</h3><p className="byline">{item.book.authors.join(", ")||"Unknown author"}</p><small>{item.book.page_count?`${item.book.page_count} pages`:"Page count unavailable"}</small></div><div className="vote-actions"><button className={item.votes.some(v=>v.user_id===session.user.id)?"voted":""} onClick={()=>toggleVote(item.book.id)}>♥ {item.votes.length}</button>{activeClub.host_id===session.user.id&&<button className="secondary" onClick={()=>chooseBook(item.book.id)}>Choose</button>}{(activeClub.host_id===session.user.id||item.nominated_by===session.user.id)&&<button className="remove-action" onClick={()=>removeShortlistBook(item)}>Remove</button>}</div></article>)}</div></>}</section></div>}
